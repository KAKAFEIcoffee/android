...
public class AudioMngHelper {
     ...
     * @return  改完后的音量值
     */
    public int setVoice100_s(float num) {
        int a =(int) Math.round((-num + 100*get100CurrentVolume())*0.01*getSystemMaxVolume()*0.01);
        a = a<=0 ? 0 : a;
        a = a>=VOICE_MAX ? VOICE_MAX : a;
        audioManager.setStreamVolume(NOW_AUDIO_TYPE,a,0);
        return get100CurrentVolume();
